...
import { ModelSelectorPopover, ModelSelectorPopoverUnpaid } from "@/components/dialog-select-model"
import { PromptSelectorPopover } from "@/features/prompts/components/prompt-selector"
...
export const PromptInput: Component<PromptInputProps> = (props) => {
  ...
  const getCaretState = () => {
    // Check if we're in a browser environment
    if (typeof window === 'undefined' || !window.getSelection) {
      return { collapsed: false, cursorPosition: 0, textLength: promptLength(prompt.current()) }
    }
  ...
  const handleSlashSelect = (cmd: SlashCommand | undefined) => {
    ...
    if (cmd.type === "custom") {
      ...
      requestAnimationFrame(() => {
        editorRef.focus()
        // Check if we're in a browser environment
        if (typeof document === 'undefined' || typeof window === 'undefined' || !window.getSelection) {
          return
        }
  ...
  return (
    <div class="relative size-full _max-h-[320px] flex flex-col gap-3">
      ...
      >
        ...
        <div class="relative">
          {/* History navigation buttons - left side */}
          <Show when={history.entries.length > 0 || shellHistory.entries.length > 0}>
            <div class="absolute left-1 top-1/2 -translate-y-1/2 flex flex-col gap-0.5 opacity-40 hover:opacity-100 transition-opacity z-50 pointer-events-auto">
              <Tooltip content="Previous prompt (↑)">
                <button
                  type="button"
                  onClick={(e) => {
                    e.preventDefault()
                  ...
                  disabled={
                    store.mode === "shell"
                      ? store.historyIndex >= shellHistory.entries.length - 1
                      : store.historyIndex >= history.entries.length - 1
                  }
                  class="p-0.5 hover:bg-background-stronger disabled:opacity-20 disabled:cursor-not-allowed transition-colors"
                  aria-label="Previous prompt"
                >
                  <Icon name="chevron-down" class="w-3 h-3 text-foreground-weaker rotate-180" />
                </button>
              </Tooltip>
              <Tooltip content="Next prompt (↓)">
                <button
                  type="button"
                  onClick={(e) => {
                    e.preventDefault()
                ...
                >
                  <Icon name="chevron-down" class="w-3 h-3 text-foreground-weaker" />
                </button>
              </Tooltip>
            </div>
          </Show>
          <div class="max-h-[240px] overflow-y-auto overflow-x-visible" ref={(el) => (scrollRef = el)}>
            <div
              ...
              classList={{
                "select-text": true,
                "w-full pl-12 pr-12 py-3 text-14-regular text-text-strong focus:outline-none whitespace-pre-wrap": true,
          ...
          <Show when={!prompt.dirty()}>
            <div class="absolute top-0 inset-x-0 pl-12 pr-12 py-3 text-14-regular text-text-weak pointer-events-none whitespace-nowrap truncate">
              {store.mode === "shell"
                ? language.t("prompt.placeholder.shell")
                : commentCount() > 1
                  ? language.t("prompt.placeholder.summarizeComments")
              ...
              <Match when={store.mode === "normal"}>
                ...
                <Show
                  ...
                  fallback={
                    ...
                    >
                      <ModelSelectorPopoverUnpaid triggerAs={Button} triggerProps={{ variant: "ghost" }}>
                        {(open) => (
                          <>
                ...
                <Tooltip placement="top" title="Select System Prompt">
                  <PromptSelectorPopover triggerAs={Button} triggerProps={{ variant: "ghost" }}>
                    {(open, displayName) => (
                      <>
                        <Icon name="file-lines" class="size-4 shrink-0" />
            ...
            <Tooltip
              ...
              value={
                working()
                  ...
                  : `${language.t("prompt.action.send")} (Enter)`
              }
            >
              <IconButton
                type="submit"
