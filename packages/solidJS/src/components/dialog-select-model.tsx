...
/**
 * Popover version of model selector for users without paid providers.
 * Shows free models and popular providers to connect.
 */
export function ModelSelectorPopoverUnpaid<T extends ValidComponent = "div">(props: {
  children?: JSX.Element | ((open: boolean) => JSX.Element)
  triggerAs?: T
  triggerProps?: ComponentProps<T>
}) {
  ...
  return (
    ...
    >
      ...
      <Kobalte.Portal>
        <Kobalte.Content
          ref={(el) => setStore("content", el)}
          class="w-72 max-h-96 flex flex-col rounded-md border border-border-base bg-surface-raised-stronger-non-alpha shadow-md z-50 outline-none overflow-y-auto"
          data-component="model-popover-content"
          onEscapeKeyDown={(event) => {
            setStore("dismiss", "escape")
            setStore("open", false)
        ...
        >
          ...
          <div class="flex-1 min-h-0 p-1">
            ...
            <List
              ...
              current={local.model.current()}
              key={(x) => `${x.provider.id}:${x.id}`}
              groupBy={() => language.t("dialog.model.unpaid.freeModels.title")}
              itemWrapper={(item, node) => (
                <Tooltip
                  class="w-full"
                  placement="right-start"
            ...
            <div class="mt-2 pt-2 border-t border-border-weak-base">
              <div class="px-2 py-1 text-12-regular text-text-weak">{language.t("dialog.model.unpaid.addMore.title")}</div>
              <List
                key={(x) => x?.id}
              ...
              >
                {(i) => (
                  <div class="w-full flex items-center gap-x-2 text-13-regular">
                    <ProviderIcon data-slot="list-item-extra-icon" id={i.id as IconName} class="size-4" />
                    <span class="truncate">{i.name}</span>
              ...
              <button
                type="button"
                class="w-full flex items-center gap-2 px-2 py-1.5 text-13-regular text-text-weak hover:bg-surface-raised-base rounded"
                onClick={handleConnectProvider}
              >
