...
import { Installation } from "../../cli/infrastructure/installation"
...
export namespace Session {
  ...
  export const updateDirectory = fn(
    z.object({
      sessionID: Identifier.schema("session"),
      directory: z.string(),
    }),
    async (input) => {
      ...
      const session = await Storage.update<Info>(
        ["session", Instance.project.id, input.sessionID],
        (draft) => {
          draft.directory = input.directory
          draft.time.updated = Date.now()
        }
      )
...
export { MessageV2 } from "./message-v2"
export { SessionPrompt } from "./prompt"
