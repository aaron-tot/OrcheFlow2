...
export namespace Provider {
  ...
  const state = Instance.state(async () => {
    ...
    for (const [providerID, provider] of configProviders) {
      ...
      log.info("merging config provider", { providerID, hasInDatabase: !!database[providerID], hasInProviders: !!providers[providerID] })
      mergeProvider(providerID, partial)
      log.info("after merge", { providerID, nowInProviders: !!providers[providerID], modelCount: providers[providerID] ? Object.keys(providers[providerID].models).length : 0 })
    }
    ...
    const finalProviderIDs = Object.keys(providers)
    log.info("final provider list", { count: finalProviderIDs.length, hasOllamaLocal: finalProviderIDs.includes("ollama-local"), ollamaProviders: finalProviderIDs.filter(id => id.includes("ollama")) })
